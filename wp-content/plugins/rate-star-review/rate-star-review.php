...
static function reviewCard($post, $rating_max = 5, $contentType = true)
{
   ...
   <div id="rating" class="ui large star rating readonly" data-rating="' . $rating . '" data-max-rating="' . $rating_max . '"></div>
	...
	//Andy edited (from $user->user_nicename to $user->display_name) (enabled herf for user profile url)
					$htmlCode .= '<div class="extra content">
    <div class="right floated author">
      <img class="ui avatar image" src="' . get_avatar_url($post->post_author). '"> ' . '<a href="' . bbp_get_user_profile_url($user->ID) . '">' . $user->display_name. '</a>' . '
    </div>
          <span class="date">' . get_the_date("Y-m-d",$post->ID) . '
      </span>
